...
public class ListenerBuscarActividades {
	...
	private void buscar(String termino) {
		...
		try {
			this.listaActividades = FXCollections.observableArrayList(dao.buscar(termino));
			this.listView.setItems(listaActividades);
			this.listView.setCellFactory(searchCell -> new ListenerActivitySearchCell(usuario));
		}catch(Exception ex) {
